...
class ArmBernoulli(StochasticArm):
...
class ArmExp(StochasticArm):
    ...
    def __init__(self, L, B=1., random_state=0):
        ...
        super(ArmExp, self).__init__(
            # mean=1 / (L * (1 - np.exp(-L * B))),
            mean=v_m / (1 - np.exp(-L * B)),  # correction
...
class ArmTruncatedGaussian1D(StochasticArm):

    def __init__(self, mu, sigma, random_state=0):
        self.mu = mu
        self.sigma = sigma
        super(ArmGaussian1D, self).__init__(mean=mu, variance=sigma, random_state=random_state)

    def sample(self):
        return min(1, max(0, self.sigma * np.random.randn() + self.mu))


class ArmGaussian1D(StochasticArm):
